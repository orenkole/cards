...
loginForm.element.addEventListener("submit", (e) => {
  ...
  const loginRequest = new Request();
	const body = {email: emailInput.element.value, password: passwordInput.element.value}
  const tokenPromise = loginRequest.sendRequest({body: body,	path: "login",	method: "POST"});
  tokenPromise
    .then(onLoginSuccess)
